...
namespace TheLiquidFire.UI
{
    ...
    public class TableViewCell : MonoBehaviour
    {
        ...
        private Tweener ApplyPosition(Panel.Position pos)
        {
            var tweener = panel.SetPosition(pos, true);
            tweener.duration = 0.5f;
            return tweener;
        }

        private Panel.Position GetPosition(string posName, TextAnchor ownAnchor, TextAnchor parentAnchor)
        {
            var position = panel[posName];
            if (position == null)
            {
