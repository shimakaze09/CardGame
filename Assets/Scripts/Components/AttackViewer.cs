...
public class AttackViewer : MonoBehaviour
{
    ...
    private void OnValidateAttack(object sender, object args)
    {
        var action = sender as AttackAction;
        action.perform.viewer = OnPerformAttack;
    }

    private IEnumerator OnPerformAttack(IContainer game, GameAction action)
    {
        ...
        var tweener = attacker.transform.MoveTo(target.transform.position + new Vector3(0, 1, 0), 0.5f,
